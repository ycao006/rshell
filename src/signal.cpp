...
int piping_f = 0;
int err_f = 0;
vector<int>pid_Group;
vector<int>temp_pid;
//string promptReform(char*);
...
void sigC(int signum, siginfo_t * info, void *ptr){
	if(pid_f != -1){
		cout<<"kill pid: "<<pid_f<<endl;
		if( -1 == kill(pid_f,SIGKILL)){
			perror("There is a error with kill() ");
			exit(1);
		}
	}	
	cout<<endl;
...
void sigZ(int signum, siginfo_t * info, void *ptr){
	if(pid_f != -1){
		pid_Group.push_back(pid_f);
		if(-1 == kill(pid_f,SIGSTOP)){
			perror("There is a error with kill() ");
			exit(1);
		}
	}
...
int main(){
		...
		ctrlC.sa_sigaction = sigC;
		ctrlC.sa_flags = SA_SIGINFO;
		sigaction(SIGINT, &ctrlC, NULL);
		
		ctrlZ.sa_sigaction = sigZ;
...
Origin:
		...
		currPath = getenv("PWD");
		currPathStr = currPath;
		getcwd(tmpprompt,512);
		prompt = tmpprompt;
		home = getenv("HOME");
		string homeStr = home;
		size_t pos = 0;
		pos = prompt.find(homeStr);
		if(pos != std::string::npos){
			prompt = prompt.replace(pos, homeStr.size(),"~"); 
		}
		prompt = prompt + " $ ";
		cout<< prompt;
...
checkInput:
		...
		while((commandList[i][0] != '\0')&&(!errorFlag1)){
			...
			if(strcmp(commandList[i],"||") == 0){
				...
				if(strcmp(temp[0],"cd") == 0 ){
					if(temp[1] == '\0'){// only cd exit
						char* newPath = getenv("HOME");
						if(-1 == chdir(newPath)){
							perror("there was an error chdir()");
							goto Origin;
						}
						setenv("OLDPWD", currPathStr.c_str(),true);
						setenv("PWD", newPath,true);
					}
					else if(temp[1][0] == '/'){// only cd exit
						char* newPath = temp[1];
						if(-1 == chdir(temp[1])){
							perror("there was an error chdir()");
							goto Origin;
						...
						setenv("OLDPWD", currPathStr.c_str(),true);
						setenv("PWD",oldPath,true);
					}
					else{
						string pathTemp=temp[1];
						string tempPathStr= currPathStr+"/"+pathTemp;
						if(-1 == chdir(tempPathStr.c_str())){
							perror("there was an error with chdir()");
							goto Origin;
						}
						setenv("OLDPWD",currPathStr.c_str(),true);	
						currPathStr = tempPathStr;
						setenv("PWD",currPathStr.c_str(),true);	
					}
				...
				if(strcmp(temp[0],"fg") == 0){
					if(temp[1] == '\0'){
						if(pid_Group.size() > 0){
							if(-1 == kill(pid_Group.at(pid_Group.size()-1) , SIGCONT)){
								perror("There is a error with kill()");
								exit(1);	
							}	
						//	temp_pid.push_back ( pid_Group.at(pid_Group.size()-1) );
							//new 	
							int fg_result_wait=0;
							do{
								//result_wait = wait(&status);//wait wait for the child process
								pid_f = pid_Group.at(pid_Group.size()-1); 
								fg_result_wait = waitpid(pid_Group.at(pid_Group.size()-1) , &status ,WUNTRACED);
								if(errno == EINTR){
									err_f = -2;
									//	return -2;	
								}
							}while((fg_result_wait == -1) && (errno == EINTR));		
					
							if((fg_result_wait == -1) && (errno != EINTR)){
							...
							cout<<"There is no job, no fg needed"<<endl;
						}
						for (unsigned int I = 0; I < maxCap; ++I) {
							delete[] commandList[I];
							commandList[I]=0;
						...
						cout<<"Invlid input related to fg line:553"<<endl;
						for (unsigned int I = 0; I < maxCap; ++I) {
						delete[] commandList[I];
						commandList[I]=0;
						}
				...
				if(strcmp(temp[0],"bg") == 0){
					if(temp[1] == '\0'){
						if(pid_Group.size() > 0){
							if(-1 == kill(pid_Group.at(pid_Group.size()-1) , SIGCONT)){
								perror("There is an error wiht kill() ");
								exit(1);
							}	
						}

						else{
							cout<<"There is no job, no bg needed"<<endl;
						}
						for (unsigned int I = 0; I < maxCap; ++I) {
							delete[] commandList[I];
							commandList[I]=0;
						...
						cout<<"Invlid input related to bg line:553"<<endl;
						for (unsigned int I = 0; I < maxCap; ++I) {
						delete[] commandList[I];
						commandList[I]=0;
						}
				...
				piping_f = piping(temp,pass,status);	
				if(-1 == piping_f ){
					for (unsigned int I = 0; I < maxCap; ++I) {
						delete[] commandList[I];
						commandList[I]=0;
				...
				if(err_f == -2){ //check ctrlc happens
					for (unsigned int I = 0; I < maxCap; ++I) {
						delete[] commandList[I];
						commandList[I]=0;
					}
				...
				if(strcmp(temp[0],"fg") == 0){
					if(temp[1] == '\0'){
						if(pid_Group.size() > 0){
							if(-1 == kill(pid_Group.at(pid_Group.size()-1) , SIGCONT)){
								perror("There is an error with kill()");
								exit(1);
							}	
							//new 	
							int fg_result_wait=0;
			...
			if(commandList[i][0]=='\0'){
				...
				if(strcmp(temp[0],"fg") == 0){
					if(temp[1] == '\0'){
						if(pid_Group.size() > 0){
							if(-1 == kill(pid_Group.at(pid_Group.size()-1) , SIGCONT)){
								perror("There is error with kill() ");
								exit(1);
							}	
							//new 	
							int fg_result_wait=0;
	...
	//	int status;
		...
		if(pid == -1){
			...
			if(-1 == sigaction(SIGINT, &ignoreC, NULL)){
				perror("there was an error with sigaction()");
				exit(1);	
			}
			if(-1 == sigaction(SIGTSTP, &ignoreC, NULL)){
				perror("there was an error with sigaction()");
				exit(1);	
			}		
			...
			do{
				//result_wait = wait(&status);//wait wait for the child process
				result_wait = waitpid(-1,&status,WUNTRACED);
				if(errno == EINTR){
					err_f = -2;
				}
			}while((result_wait == -1) && (errno == EINTR));		
			if((result_wait == -1) && (errno != EINTR)){
				perror("There was an error in wait line35");
				exit(1);
			}	
...
int piping(char** str1, bool& pass, int& status, int savestdin){
	...
	if(!foundPipe){
		t_sys_f = syscall(s1,pass,status);
		...
		if(pid == -1){
			...
			t_sys_f =  syscall(LHS,pass, status);
			if(-1 == t_sys_f ){ // call syscall to run command
	...
	// as a result, we only save the original stdin at the first time. 		
			...
			t_piping_f = piping(RHS,pass,status,savestdin);
			if(-1 == t_piping_f){// recursively do pipe
